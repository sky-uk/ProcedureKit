...
internal class ResilientNetworkRecovery<T: Procedure> where T: ResultInjection, T.Result == HTTPResult<Data> {
 ...
 */
open class ResilientNetworkProcedure<T: Procedure>: RetryProcedure<T> where T: ResultInjection, T.Result == HTTPResult<Data> {

    internal private(set) var recovery: ResilientNetworkRecovery<T>

    public init<NetworkIterator: IteratorProtocol>(behavior: ResilientNetworkBehavior, iterator: NetworkIterator) where T == NetworkIterator.Element {
        ...
        super.init(max: recovery.max, wait: recovery.wait, iterator: tmp, retry: handler)
    }

    public convenience init(behavior: ResilientNetworkBehavior, body: @escaping () -> T?) {
        self.init(behavior: behavior, iterator: AnyIterator(body))
    }
